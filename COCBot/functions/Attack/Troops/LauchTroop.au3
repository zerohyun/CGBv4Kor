Func LauchTroop($troopKind, $nbSides, $waveNb, $maxWaveNb, $slotsPerEdge = 0)
	...
	if $maxWaveNb = 1 Then $waveName = "총"
			...
			For $numWave = 0 To UBound($listListInfoDeployTroopPixel) - 1
				...
				For $i = 0 To UBound($listInfoDeployTroopPixel) - 1
					...
					Else
						...
						If _Sleep(100) Then Return
						Local $waveName = "첫번째"
						If $numWave + 1 = 2 Then $waveName = "두번째"
						If $numWave + 1 = 3 Then $waveName = "세번째"
						If $numWave + 1 = 0 Then $waveName = "마지막"
						SetLog("'" & $infoPixelDropTroop[4] & "' 의 " & $waveName & " 공격 :" & $infoPixelDropTroop[5] & " 마리 투입", $COLOR_GREEN)
