		...
		If $iChkUseCCBalanced = 1 Then
			If Number($TroopsReceived) <> 0 Then
				If Number(Number($TroopsDonated) / Number($TroopsReceived)) >= (Number($iCmbCCDonated) / Number($iCmbCCReceived)) Then
					SetLog("'클랜성' 사용 - 지원(" & $TroopsDonated & ") / 요청(" & $TroopsReceived & ") >= " & $iCmbCCDonated & "/" & $iCmbCCReceived, $COLOR_BLUE)
					Click(68 + (72 * $slot), 595, 1, 500,"#0086")
					If _Sleep(500) Then Return
					Click($x, $y,1,0,"#0087")
				Else
					SetLog("'클랜성' 사용안함, 지원(" & $TroopsDonated & ") / 요청(" & $TroopsReceived & ") < " & $iCmbCCDonated & "/" & $iCmbCCReceived, $COLOR_BLUE)
				EndIf
			Else
				If Number(Number($TroopsDonated) / 1) >= (Number($iCmbCCDonated) / Number($iCmbCCReceived)) Then
					SetLog("'클랜성' 사용, 지원(" & $TroopsDonated & ") / 요청(" & $TroopsReceived & ") >= " & $iCmbCCDonated & "/" & $iCmbCCReceived, $COLOR_BLUE)
					Click(68 + (72 * $slot), 595, 1, 500,"#0088")
					If _Sleep(500) Then Return
					Click($x, $y,1,0,"#0089")
				Else
					SetLog("'클랜성' 사용안함, 지원(" & $TroopsDonated & ") / 요청(" & $TroopsReceived & ") < " & $iCmbCCDonated & "/" & $iCmbCCReceived, $COLOR_BLUE)
			...
			EndIf
		Else
			SetLog("'클랜성' 사용", $COLOR_BLUE)
			Click(68 + (72 * $slot), 595, 1, 500,"#0090")
			If _Sleep(500) Then Return
			Click($x, $y,1,0,"#0091")
		EndIf
	EndIf
