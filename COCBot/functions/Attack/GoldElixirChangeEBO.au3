...
Func GoldElixirChangeEBO()
	...
	If _ColorCheck(_GetPixelColor($aWonTwoStar[0], $aWonTwoStar[1], True), Hex($aWonTwoStar[2], 6), $aWonTwoStar[3]) Then
		If $ichkEndTwoStars = 1 Then
			SetLog("별 두개를 얻어 전투를 종료합니다.", $COLOR_GREEN)
			$exitTwoStars = 1
			$z = 0
		EndIf
	EndIf
	...
	If _ColorCheck(_GetPixelColor($aWonOneStar[0], $aWonOneStar[1], True), Hex($aWonOneStar[2], 6), $aWonOneStar[3]) Then
		If $ichkEndOneStar = 1 Then
			SetLog("별 한개를 얻어 전투를 종료합니다.", $COLOR_GREEN)
			$exitOneStar = 1
			$z = 0
		EndIf
	EndIf
	...
	While TimerDiff($iBegin) < $z
		...
		If $Gold2 = "" and $Elixir2 = "" and $DarkElixir2= "" Then
			SetLog("확인 - [G]: " & $Gold2 & " [E]: " & $Elixir2 & " [DE]: " & $DarkElixir2 & " |  전투종료 ", $COLOR_BLUE)
		Else
			SetLog("확인 - [G]: " & $Gold2 & " [E]: " & $Elixir2 & " [DE]: " & $DarkElixir2 & " | " & StringReplace(StringFormat("%2.1f", $txtDiff), "-", "") & " 초 후 전투종료", $COLOR_BLUE)
		EndIf
		...
		If $ichkEndNoResources = 1 And Number($Gold2) = 0 And Number($Elixir2) = 0 And Number($DarkElixir2) = 0 Then
			SetLog("더이상 약탈할 자원이 없어 전투를 종료합니다.", $COLOR_GREEN)
	...
	If $ichkEndNoResources = 1 And Number($Gold2) = 0 And Number($Elixir2) = 0 And Number($DarkElixir2) = 0 Then
		SetLog("더이상 약탈할 자원이 없습니다. 전투를 종료합니다. ", $COLOR_GREEN)
		If _Sleep(Random(500, 2500, 1)) Then Return ;wait random seconds... antiban purpose...
		Return False
	EndIf
	...
	If (Number($Gold1) = Number($Gold2) And Number($Elixir1) = Number($Elixir2) And Number($DarkElixir1) = Number($DarkElixir2)) Then
		SetLog("약탈할 자원량이 변하지 않아 전투를 종료합니다.", $COLOR_GREEN)
		Return False
	Else
