...
Func AttackReport()
	...
	If _ColorCheck(_GetPixelColor(459, 372, True), Hex(0x433350, 6), 20) Then ; if the color of the DE drop detected
		...
		EndIf
		SetLog("Loot: [G]: " & _NumberFormatLog($lootGold) & " [E]: " & _NumberFormatLog($lootElixir) & " [DE]: " & _NumberFormatLog($lootDarkElixir) & " [T]: " & $lootTrophies, $COLOR_GREEN)
	...
	If $lootTrophies >= 0 Then
		If _ColorCheck(_GetPixelColor(678, 418, True), Hex(0x030000, 6), 30) Then
			...
			SetLog("Bonus [G]: " & _NumberFormatLog($BonusLeagueG) & " [E]: " & _NumberFormatLog($BonusLeagueE) & " [DE]: " & _NumberFormatLog($BonusLeagueD), $COLOR_GREEN)
		Else
			If _sleep(150) Then Return
			$BonusLeagueG = getResourcesBonus(590, 340)
			$BonusLeagueG = StringReplace($BonusLeagueG, "+", "")
			...
			$BonusLeagueE = getResourcesBonus(590, 371)
			$BonusLeagueE = StringReplace($BonusLeagueE, "+", "")
			SetLog("Bonus [G]: " & _NumberFormatLog($BonusLeagueG) & " [E]: " & _NumberFormatLog($BonusLeagueE), $COLOR_GREEN)
		EndIf
		$LeagueShort = "--"
		For $i = 0 To 15
			If _sleep(150) Then Return
