...
Func checkObstacles() ;Checks if something is in the way for mainscreen
	...
	If _ImageSearchArea($device, 0, 237, 321, 293, 346, $x, $y, 80) Then
		If $sTimeWakeUp > 3600 Then
			SetLog("다른기기접속 : " & Floor(Floor($sTimeWakeUp / 60) / 60) & " 시간 " & Floor(Mod(Floor($sTimeWakeUp / 60), 60)) & " 분 " & Floor(Mod($sTimeWakeUp, 60)) & " 초 후에 재접속합니다.", $COLOR_RED)
			PushMsg("AnotherDevice3600")
		ElseIf $sTimeWakeUp > 60 Then
			SetLog("다른기기접속 : " & Floor(Mod(Floor($sTimeWakeUp / 60), 60)) & " 분 " & Floor(Mod($sTimeWakeUp, 60)) & "  초 후에 재접속합니다.", $COLOR_RED)
			PushMsg("AnotherDevice60")
		Else
			SetLog("다른기기접속 : " & Floor(Mod($sTimeWakeUp, 60)) & "  초 후에 재접속합니다.", $COLOR_RED)
