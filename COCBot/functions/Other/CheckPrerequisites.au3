...
Func CheckPrerequisites()
	...
	If ($isNetFramework4dot5Installed = False Or $isVC2010Installed = False) Then
		If ($isNetFramework4dot5Installed = False) Then
			SetLog("The .Net Framework 4.5 가 설치되지 않았습니다.", $COLOR_RED)
			SetLog("다운로드 : https://www.microsoft.com/ko-KR/download/details.aspx?id=30653", $COLOR_RED)
		EndIf
		If ($isVC2010Installed = False) Then
			SetLog("The VC 2010 x86가 설치되지 않았습니다.", $COLOR_RED)
			SetLog("다운로드 : https://www.microsoft.com/ko-KR/download/details.aspx?id=5555", $COLOR_RED)
		EndIf
