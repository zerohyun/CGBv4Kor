		...
		If $iChkTrophyAtkDead = 1 Then
			If ($CurCamp >= ($TotalCamp * 70 / 100)) Then
				...
				While Number($TrophyCount) > Number($itxtMaxTrophyNeedCheck)
					...
					SetLog("현재 트로피 : " & $TrophyCount, $COLOR_GREEN)
					If Number($TrophyCount) > Number($itxtMaxTrophyNeedCheck) Then
						$itxtMaxTrophyNeedCheck = $itxtdropTrophy ; $itxtMinTrophy = 1650
						SetLog("떨어트릴 트로피 범위 : " & $itxtdropTrophy, $COLOR_BLUE)
						...
						If $G = True And $E = True And $GPE = True Then
							SetLog("검색 - [G]: " & _NumberFormat($searchGold) & " [E]: " & _NumberFormat($searchElixir) & " [T]: " & _NumberFormat($searchTrophy), $COLOR_BLACK, "Lucida Console")
...
Func SetTrophyLoss()
	...
	EndIf
	Setlog("트로피 손실 = " & $sTrophyLoss, $COLOR_PURPLE) ; record trophy loss
