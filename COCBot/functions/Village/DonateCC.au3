...
Func DonateCC($Check = False)
	...
	If $iPlannedDonateHoursEnable = 1 Then
		...
		If $iPlannedDonateHours[$hour[0]] = 0 Then
			SetLog("지원유닛이 설정되지 않았습니다.", $COLOR_GREEN)
	...
	While $Donate
		...
		If IsArray($DonatePixel) Then
			...
			If $DonateTroop Then
				...
				If $ClanString = "" Or $ClanString = " " Then
					SetLog("요청단어를 읽을 수 없습니다.", $COLOR_RED)
					$Donate = True
					$y = $DonatePixel[1] + 10
					ContinueLoop
				Else
					SetLog("유닛요청단어 : " & $ClanString)
				EndIf
...
Func CheckDonateTroop($Type, $aDonTroop, $aBlkTroop, $aBlackList, $ClanString)
	...
	For $i = 1 To UBound($aBlackList) - 1
		If CheckDonateString($aBlackList[$i], $ClanString) Then
			SetLog("항상지원요청금지키워드 : " & $aBlackList[$i], $COLOR_RED)
			Return False
		EndIf
	Next
	...
	For $i = 1 To UBound($aBlkTroop) - 1
		If CheckDonateString($aBlkTroop[$i], $ClanString) Then
			SetLog(NameOfTroop($Type) & " 지원금지키워드 : " & $aBlkTroop[$i], $COLOR_RED)
			Return False
		EndIf
	Next
	...
	For $i = 1 To UBound($aDonTroop) - 1
		If CheckDonateString($aDonTroop[$i], $ClanString) Then
			If $Type > $eLava Then
				Setlog("유닛조합지원키워드 : " & $aDonTroop[$i], $COLOR_GREEN)
			Else
				Setlog(NameOfTroop($Type) & " 지원 키워드 : " & $aDonTroop[$i], $COLOR_GREEN)
			EndIf
			Return True
		EndIf
	Next
...
Func DonateTroopType($Type, $quant = 8, $custom = False)
	...
	If _ColorCheck(_GetPixelColor(237 + ($Slot * 82), $DonatePixel[1] - 5 + $YComp), Hex(0x507C00, 6), 10) Or _
		...
		If $custom Then
			SetLog(NameOfTroop($Type) & " " & $quant & " 마리 지원 성공", $COLOR_GREEN)
		Else
			SetLog(NameOfTroop($Type) & " 지원 성공", $COLOR_GREEN)
		EndIf
...
;~		PureClick(237 + ($Slot * 82), $DonatePixel[1] - 10 + $YComp, $quant, 50)
	...
	ElseIf $DonatePixel[1] - 5 + $YComp > 675 Then
		Setlog(NameOfTroop($Type) & " 지원 실패 (유닛지원화면을 확인할 수 없습니다. 다음에 다시 시도합니다.)", $COLOR_RED)
	Else
		SetLog("지원가능한 " & NameOfTroop($Type) & " 유닛이 없습니다.", $COLOR_RED)
	EndIf
