			...
			Switch $icmbBotCommand
				Case 0
					If $bDonationEnabled = False Then
						SetLog("공격중지 : 온라인유지/자원수집", $COLOR_BLUE)
					ElseIf $bTrainEnabled = False Then
						SetLog("공격중지 : 온라인유지/자원수집/클랜지원", $COLOR_BLUE)
					Else
						SetLog("공격중지 : 온라인유지/자원수집/유닛훈련/클랜지원", $COLOR_BLUE)
					EndIf
					$CommandStop = 0 ; Halt Attack
					If _Sleep(500) Then Return
				Case 1
					SetLog("PC 강제종료", $COLOR_BLUE)
					If _Sleep(500) Then Return
					Shutdown(5) ; Force Shutdown
					Return True
				Case 2
					SetLog("PC 절전모드", $COLOR_BLUE)
					If _Sleep(500) Then Return
