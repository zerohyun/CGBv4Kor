...
Func PushMsg($Message, $Source = "")
	...
	Switch $Message
		...
		Case "LastRaid"
			...
			EndIf
		 Case "LastRaidTxt"
            If $pEnabled = 1 And $iAlertPBLastRaidTxt = 1 Then _Push($iOrigPushB & " | Last Raid - [G]: " &  _NumberFormat($lootGold) & " [E]: " &  _NumberFormat($lootElixir) & " [D]: " &  _NumberFormat($lootDarkElixir) & "  [T]: " & $lootTrophies)
		Case "FoundWalls"
