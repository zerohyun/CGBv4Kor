...
Func ReArm()
	...
	SetLog("재장전/재설치가 필요한지 확인합니다.", $COLOR_BLUE)
	...
	If IsArray($RearmPixel) Then
		...
		If isGemOpen(True) = True Then
			Setlog("재설치에 필요한 자원이 부족합니다.", $COLOR_RED)
			Click(585, 252,1,0,"#0227")  ; Click close gem window "X"
			If _Sleep(500) Then Return
		Else
			SetLog("재설치 성공!!", $COLOR_GREEN)
			If _Sleep(500) Then Return
		EndIf
	Endif
	...
	If IsArray($XbowPixel) Then
		...
		If _Sleep(700) Then Return
		If isGemOpen(True) = True Then
			Setlog("'대형석궁' 재장전에 필요한 자원이 부족합니다.", $COLOR_RED)
			Click(585, 252,1,0,"#0230")  ; Click close gem window "X"
			If _Sleep(500) Then Return
		Else
			SetLog("'대형석궁' 재장전 성공!!", $COLOR_GREEN)
			If _Sleep(500) Then Return
		EndIf
	EndIf
	...
	If IsArray($InfernoPixel) Then
		...
		If _Sleep(700) Then Return
		If isGemOpen(True) = True Then
			Setlog("'인페르노타워' 재장전에 필요한 자원이 부족합니다.", $COLOR_RED)
			Click(585, 252,1,0,"#0233")  ; Click close gem window "X"
			If _Sleep(500) Then Return
		Else
			SetLog("'인페르노타워' 재장전 성공!!", $COLOR_GREEN)
			If _Sleep(500) Then Return
		EndIf
	EndIf
