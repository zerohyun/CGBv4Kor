...
Func LocateUpgrades()
	...
	While 1
		...
		EndIf
		For $icount = 0 To 5
			$stext = "'건물위치설정'버튼을 누른 후 업그레이드할 건물/영웅을 클릭하세요" & @CRLF & @CRLF & "위치설정이 끝나면 '종료'버튼을 누르세요" & @CRLF & @CRLF & "위치설정을 취소하려면 '취소'버튼을 눌러주세요" & @CRLF & @CRLF
			_ExtMsgBoxSet(1 + 64, $SS_CENTER, 0x004080, 0xFFFF00, 10, "verdana", 500)
			$MsgBox = _ExtMsgBox(0, "건물위치설정|종료|취소", "업그레이드 위치설정", $stext, 0, $frmBot)
			Switch $MsgBox
				...
				Case 2 ; No! we are done!
					If $icount = 0 Then ; if no upgrades located, reset all values and return
						Setlog("업그레이드 위치설정 취소", $COLOR_FUCHSIA)
						btnResetUpgrade()
						Return False
					EndIf
					ExitLoop
...
Func CheckUpgrades() ; Valdiate and determine the cost and type of the upgrade and change GUI boxes/pics to match
	...
	$stext = "업그레이드할 건물/영웅의 정보를 확인합니다. 마우스 커서를 블루스택 바깥쪽으로 이동시켜 주세요. 감사합니다!!"
