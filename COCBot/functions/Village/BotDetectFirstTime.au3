...
If $Is_ClientSyncError = False Then
	...
	SetLog("건물 위치를 확인합니다.", $COLOR_BLUE)
...
;~ 	EndIf
	...
	If $listResourceLocation = "" Then
		...
		If UBound($PixelElixirHere) > 0 Then
			SetLog("엘릭서정제소 : " & UBound($PixelElixirHere) & " 개")
		EndIf
		...
		If UBound($PixelDarkElixirHere) > 0 Then
			SetLog("다크엘릭서 정제소 : " & UBound($PixelDarkElixirHere) & " 개")
		EndIf
