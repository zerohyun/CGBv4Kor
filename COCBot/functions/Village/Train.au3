...
Func Train()
	...
	If $lastbarrack = 0 Then
		Setlog("사용할 수 없는 '훈련소'입니다. 훈련을 시작할 수 없습니다.")
	...
	If $fullArmy Then
		...
		SetLog("'집합소'가 가득 찼습니다.", $COLOR_RED)
	...
	If _GUICtrlComboBox_GetCurSel($cmbTroopComp) = 8 Then
		...
		While isBarrack()
			...
			EndSwitch
			If $OutOfElixir = 1 Then
				Setlog("유닛훈련에 필요한 '엘릭서'가 부족합니다!", $COLOR_RED)
				Setlog("'공격중지'전환 : 온라인유지", $COLOR_RED)
				$ichkBotStop = 1 ; set halt attack variable
				$icmbBotCond = 16 ; set stay online
				;GUICtrlSetState($chkBotStop, $GUI_CHECKED)
				;_GUICtrlComboBox_SetCurSel($cmbBotCond, $icmbBotCond)
    ...
    If Not $isDarkBuild Or ($BarrackDarkStatus[0] = False And $BarrackDarkStatus[1] = False) Then
	$TrainLoopCounter+=1
    if $TrainLoopCounter>=3 Then
           If ($CurCamp >= ($TotalCamp * $fulltroop* 98/ 100)) Then
    $fullArmy = True
           EndIf
           Return
    EndIf
...
Func IsOdd($chkbrrnum)
	if (Mod($chkbrrnum, 2) >= 1)  Then
		return True
	Else
		return False
	EndIf
